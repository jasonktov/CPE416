...
class Filter(Node):
    ...
    def timer_callback(self):
        ...
        quat = euler_to_quaternion(0, 0, self.state[2])
        ...
        self.get_logger().info('Publishing: "%f" "%f" "%f"' % (self.state[0], self.state[1], self.state[2]))
    ...
    def predict(self, imu:Imu, dt):
        ...
        theta_pred = math.atan2(math.sin(self.state[2] + w * dt), math.cos(self.state[2] + w * dt))
        self.state[2] = theta_pred
    ...
    def update(self, odom:PoseWithCovariance):
        ...
        Z = np.array([odom.pose.position.x, odom.pose.position.y, theta_p])
        self.get_logger().info("Z: %f, %f, %f" % (Z[0], Z[1], Z[2]))
        ...
        nu = Z - self.state
        self.state = self.state + (K @ nu)
        self.get_logger().info("new state: %f %f %f" % (self.state[0], self.state[1], self.state[2]))
